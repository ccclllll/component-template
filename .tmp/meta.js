import CAFTUserCardMeta from 'D:\\gitlab\\materials-react\\packages\\CAFTUserCard\\lowcode\\c-a-f-t-user-card\\meta'
...
function fillRealVersion(meta, packageName = 'CAFTUserCard', version = '0.1.0', basicLibraryVersion={"@alifd/next":"1.25.23","@alifd/meet":"2.6.3","antd":"4.17.3"}) {
  if (!meta || !version) {
    return meta;
  }
  const { npm } = meta;
...
const componentMetas = [CAFTUserCardMeta];
const components = [];
const npmInfo = {};
...
if (!execCompile) {
  window.CAFTUserCardMeta = { components, componentList };
}